...
| OpenAI provider authentication mode | OpenAI Codex OAuth route via `openai-codex/...` models and `provider login` command; standard OpenAI remains API-key based | keep optional local `codex` CLI-backed OpenAI route via `providers.openai.useCodexCli=true` for `openai/...` models (alongside upstream OAuth route) | enterprise seat-auth fallback and headless/runtime flexibility | provisional | every upstream sync and provider/runtime-profile PRs | open |
